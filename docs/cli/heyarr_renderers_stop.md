## heyarr renderers stop

Stop a renderer and release the content

### Synopsis

End playback. The session keeps the position it had reached, so it can be resumed later from the catalog.

```
heyarr renderers stop <renderer> [flags]
```
