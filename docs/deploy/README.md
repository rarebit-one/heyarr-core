...
- **[`reference-linux-host.md`](reference-linux-host.md)** — the reference
  ...
  `systemd-analyze security` score, and what the absence of block cloning on
  that machine actually costs.
...
`reference-linux-host.md` for running it against a packaged binary on a machine
with no Go toolchain.
