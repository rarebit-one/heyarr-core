...
// Constructor builds the webfeed adapter for providers.BuildWith.
//
// It lives here rather than in the registry for the same reason the TVDB,
// indexer, download, podcast and youtube constructors do: internal/providers
// cannot import this package, because this one imports IT for the Provider and
...
func Constructor(r providers.Resolved, now func() time.Time) (providers.Provider, bool, error) {
	if r.Kind != providers.KindWebFeed {
		return nil, false, nil
	}
	// No endpoint and no credential: a feed's address is the followed source's
	// own FeedRef, handed to Enumerate per poll, and a public feed authenticates
	// nothing. So construction takes only the name and the clock.
