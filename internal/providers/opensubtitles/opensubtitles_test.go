...
func TestRateLimiterSpacesRequests(t *testing.T) {
	...
	rl := newRateLimiter(100*time.Millisecond, clk.now, sleep)
...
func TestRateLimiterHonoursContext(t *testing.T) {
	rl := newRateLimiter(time.Hour, nil, nil) // real sleep
	if err := rl.wait(context.Background()); err != nil {
...
func TestRateLimiterDisabled(t *testing.T) {
	var slept int
	rl := newRateLimiter(0, nil, func(context.Context, time.Duration) error { slept++; return nil })
	for i := 0; i < 5; i++ {
		if err := rl.wait(context.Background()); err != nil {
			t.Fatal(err)
		}
