...
// unauthenticated renderer route (ADR-0037). They are returned apart rather
...
func (c *Controller) mounts(db *sqlite.DB, store *auth.Store, blobStore cas.Store, eventLog *events.Log, members *membership.Store, selfPeerID string) (apiMounts, publicMounts []httpapi.MountFunc, err error) {
	...
	// The renderer capability secret (ADR-0037), read before the resource API
	...
	// The renderer route (ADR-0037). Its signing secret belongs to the node
