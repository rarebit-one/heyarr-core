...
func TestRegeneratingTheFixturesIsDeterministic(t *testing.T) {
	ffmpeg := genmediaFFmpeg(t) // skips when the generator would have no ffmpeg
	...
	local := localFFmpegVersion(t, ffmpeg)
	if local != generated {
...
// genmediaFFmpeg resolves the ffmpeg scripts/genmedia.sh will run — $FFMPEG,
// else the pinned toolchain's — and skips when there is none. It deliberately
// does not fall back to an ffmpeg on PATH: the script does not either, so a
// system ffmpeg would un-skip this test only for the script to exit 1.
func genmediaFFmpeg(t *testing.T) string {
	t.Helper()
	path := os.Getenv("FFMPEG")
	if path == "" {
	...
	if fi, err := os.Stat(path); err != nil || fi.IsDir() || fi.Mode()&0o111 == 0 {
		t.Skipf("no ffmpeg for scripts/genmedia.sh at %s; run scripts/toolchain.sh", path)
	}
	return path
}

func localFFmpegVersion(t *testing.T, path string) string {
	t.Helper()
	// #nosec G204 -- the ffmpeg genmedia.sh itself runs, resolved above
