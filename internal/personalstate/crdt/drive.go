...
//
// STATUS: SKELETON for #538 / ADR-0095. Convergence, version history, trash and
// the two-way conflict (multiple live heads) are implemented and order-independent.
// The parts left for W2, marked TODO(#538):
//   1. conflicted-copy RELOCATION — turning multiple live heads into visible files
//      at a deterministic derived path, and a dotted version vector so chained /
//      multi-way conflicts and concurrent put-vs-delete resolve faithfully (this
//      skeleton settles put-vs-delete by the total order, not by "both survive");
//   2. GC/retention — when a superseded or trashed blob's reference is dropped so
//      the CAS can reclaim it (ADR-0018).
...
// IDENTICAL key for "the same path" and never conflict spuriously: forward-slash
// separators, cleaned, leading slash trimmed, case-sensitive.
//
// TODO(#538): also apply Unicode NFC normalisation (golang.org/x/text/unicode/norm)
// — the ADR names it as part of the wire rule; it is omitted here only to keep the
// skeleton stdlib-only, and MUST land before any change crosses the wire.
...
func (d *Drive) Put(p, blob string, size, mtime int64) DriveChange {
	np := normalisePath(p)
	var base posKey
...
// tombstoned path's covered blobs) is retained the same way; a dedicated Trashed()
// reader is TODO(#538) alongside the retention policy that eventually drops these.
