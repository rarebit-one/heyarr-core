...
// something else, and every credential in it goes into the log.
//
// The struct here is local rather than a real configuration type, because this
// package must not import the ones that hold credentials — they import it. The
// claim under test is about the TYPE's behaviour inside any struct, and a
// stand-in with a Value field is exactly that claim. The provider-level
// version, over the real config entry, lives in internal/providers.
func TestValueSurvivesLoggingAWholeStruct(t *testing.T) {
